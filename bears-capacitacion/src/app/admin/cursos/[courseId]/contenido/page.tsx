...
import { CourseContentStudio, type CourseContentAsset, type CourseContentModule } from "@/components/admin/course-content-studio";
...
export default async function CourseContentPage({ params }: CourseContentPageProps) {
  ...
  const { data: courseData } = await supabase.from("courses").select("id, title, slug").eq("id", courseId).maybeSingle();
  if (!courseData) notFound();

  const { data: moduleData } = await supabase.from("modules").select("id, title, order_index").eq("course_id", courseData.id).order("order_index");
  ...
  return <PortalLayout viewer={viewer} activeKey="courses"><CourseContentStudio course={courseData} courseAssets={courseAssets} modules={modules} /></PortalLayout>;
}
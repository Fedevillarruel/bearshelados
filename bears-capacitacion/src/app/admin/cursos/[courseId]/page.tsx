...
export default async function AdminCoursePage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  const [{ data: assetData }, { data: courseExamData }, { data: moduleExamData }] = await Promise.all([
    moduleIds.length ? supabase.from("assets").select("id, module_id, type, title, description, url, storage_path, duration_seconds, order_index").in("module_id", moduleIds).order("order_index") : Promise.resolve({ data: [] }),
  ...
  const assets = (assetData ?? []) as Array<{ id: string; module_id: string | null; type: BuilderAsset["type"]; title: string; description: string | null; url: string; storage_path: string | null; duration_seconds: number; order_index: number }>;
  const modules: BuilderModule[] = sourceModules.map((courseModule) => ({ id: courseModule.id, title: courseModule.title, description: courseModule.description, orderIndex: courseModule.order_index, isPublished: courseModule.is_published, assets: assets.filter((asset) => asset.module_id === courseModule.id).map((asset) => ({ id: asset.id, type: asset.type, title: asset.title, description: asset.description, url: asset.url, storagePath: asset.storage_path, durationSeconds: asset.duration_seconds, orderIndex: asset.order_index })), exams: exams.filter((exam) => exam.moduleId === courseModule.id) }));

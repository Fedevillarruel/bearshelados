...
const courseSchema = z
  .object({
    ...
    coverUrl: z
      .string()
      .trim()
      .url("Ingresá una URL de portada válida.")
      .refine(
        (value) => value.startsWith("https://"),
        "La URL de portada debe usar HTTPS.",
      )
      .nullable()
      .or(z.literal("")),
  ...
  .superRefine((value, context) => {
    if (value.coverUrl && value.coverStoragePath) {
      context.addIssue({
        ...
        message: "Elegí una URL externa o una imagen adjunta para la portada.",
      });
    }
    if (value.coverStoragePath && !value.id) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["coverStoragePath"],
        message: "Primero guardá el curso antes de adjuntar una portada.",
      });
    }
  });
...
const assetSchema = z
  ...
  .superRefine((value, context) => {
    if (
      (value.courseId && value.moduleId) ||
      (!value.courseId && !value.moduleId)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["moduleId"],
        message: "El contenido debe pertenecer a un curso o a un módulo.",
      });
    }
    ...
    if (value.type === "text" && !value.url) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["url"],
        message: "Ingresá el contenido de texto.",
      });
    }
    if (
      !value.storagePath &&
      ["video", "pdf", "image", "spreadsheet", "document", "link"].includes(
        value.type,
      ) &&
      ...
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["url"],
        message: "Ingresá una URL válida para este tipo de contenido.",
      });
    }
    if (
      value.storagePath &&
      !["video", "pdf", "image", "spreadsheet", "document"].includes(value.type)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["storagePath"],
        message:
          "Solo podés subir archivos para videos, PDFs, imágenes, planillas y documentos.",
      });
    }
    if (value.videoPosterStoragePath && value.type !== "video") {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["videoPosterStoragePath"],
        message: "Solo los videos pueden tener portada propia.",
      });
    }
    if (value.isPrimary && (value.type !== "video" || !value.moduleId)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["isPrimary"],
        message: "El video principal debe pertenecer a un módulo.",
      });
    }
  });
...
const coverUploadUrlSchema = z.object({
  ...
  fileName: z.string().trim().min(1).max(240),
  contentType: z.enum(
    coverUploadContentTypes,
    "La portada debe ser una imagen JPEG, PNG, WebP o GIF.",
  ),
  fileSize: z.number().int().positive().optional(),
});
            ...
            .array(
              z.object({
                label: z
                  .string()
                  .trim()
                  .min(1, "Cada opción necesita texto.")
                  .max(500),
                isCorrect: z.boolean(),
              }),
            )
            .min(2, "Cada pregunta necesita al menos dos opciones.")
            .max(10),
        }),
      )
  ...
  .superRefine((value, context) => {
    if (
      ...
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["moduleId"],
        message: "El examen debe pertenecer a un curso o a un módulo.",
      });
    }
    value.questions.forEach((question, index) => {
      if (question.options.filter((option) => option.isCorrect).length !== 1) {
        context.addIssue({
          ...
          message: "Cada pregunta debe tener una única respuesta correcta.",
        });
      }
    });
  });
...
export async function saveCourse(input: unknown) {
  ...
  if (course.coverStoragePath) {
    ...
    if (
      !course.coverStoragePath.startsWith(storagePrefix) ||
      !fileName ||
      fileName.includes("/")
    ) {
      return { error: "La portada no pertenece a este curso." };
    }
    const { data: storedObject, error: storageError } =
      await createAdminClient()
        .storage.from("course-media")
        .info(course.coverStoragePath);
    if (storageError || !storedObject)
      return {
        error: "La portada adjunta no existe o todavía no terminó de cargarse.",
      };
  }
  const payload = {
    title: course.title,
  ...
  if (error || !data)
    return { error: error?.message ?? "No pudimos guardar el curso." };
  if (
    previousCoverStoragePath &&
    previousCoverStoragePath !== course.coverStoragePath
  ) {
    await createAdminClient()
      .storage.from("course-media")
...
export async function deleteCourse(courseId: string) {
  ...
  if (course?.cover_storage_path)
    await createAdminClient()
      .storage.from("course-media")
      .remove([course.cover_storage_path]);
  refreshCoursePaths();
...
export async function saveModule(input: unknown) {
  ...
  const supabase = await createClient();
  const payload = {
    course_id: courseModule.courseId,
    title: courseModule.title,
    description: courseModule.description,
    order_index: courseModule.orderIndex,
    is_published: courseModule.isPublished,
  };
  ...
  const { data, error } = await request;
  if (error || !data)
    return { error: error?.message ?? "No pudimos guardar el módulo." };
...
export async function reorderModules(input: unknown) {
  await requireRole(["admin"]);
  const parsed = z
    .object({ courseId: identifier, moduleIds: z.array(identifier).min(1) })
    .safeParse(input);
  ...
  if (results.some((result) => result.error))
    return { error: "No pudimos actualizar el orden de los módulos." };
...
export async function createCourseCoverUploadUrl(input: unknown) {
  ...
  const { data, error } = await supabase.storage
    ...
    .createSignedUploadUrl(path);
  if (error || !data)
    return { error: "No pudimos preparar la subida de la portada." };
  return { data: { path: data.path, token: data.token } };
}

export async function createCourseVideoPosterUploadUrl(input: unknown) {
  await requireRole(["admin"]);
  const parsed = coverUploadUrlSchema.safeParse(input);
  if (!parsed.success)
    return {
      error:
        parsed.error.issues[0]?.message ?? "La portada del video no es válida.",
    };
  const fileName = parsed.data.fileName.replace(/[^a-zA-Z0-9._-]/g, "-");
  const path = `courses/${parsed.data.courseId}/video-posters/${crypto.randomUUID()}-${fileName}`;
  const supabase = await createClient();
  const { data: course } = await supabase
    .from("courses")
    .select("id")
...
export async function saveAsset(input: unknown) {
  ...
  if (asset.storagePath) {
    ...
    if (storageError || !storedObject)
      return {
        error: "El archivo privado no existe o todavía no terminó de cargarse.",
      };
    verifiedSizeBytes =
      storageObjectSize(storedObject.size) ??
      storageObjectSize(storedObject.metadata?.size);
  }
  const videoPosterStoragePath =
    asset.type === "video" ? (asset.videoPosterStoragePath ?? null) : null;
  if (videoPosterStoragePath) {
    const posterPrefix = `courses/${parentCourseId}/video-posters/`;
    const fileName = videoPosterStoragePath.slice(posterPrefix.length);
    if (
      !videoPosterStoragePath.startsWith(posterPrefix) ||
      !fileName ||
      fileName.includes("/")
    ) {
      return { error: "La portada del video no pertenece a este curso." };
    }
    const { data: storedPoster, error: posterError } = await createAdminClient()
      .storage.from("course-media")
      .info(videoPosterStoragePath);
    if (posterError || !storedPoster)
      return {
        error:
  ...
  const payload = {
    ...
    video_poster_storage_path: videoPosterStoragePath,
  ...
  const { data, error } = await request;
  if (error || !data)
    return { error: error?.message ?? "No pudimos guardar el contenido." };
...
export async function deleteAsset(assetId: string) {
  ...
  if (!parsed.success)
    return { error: "El contenido seleccionado no es válido." };
  const supabase = await createClient();
  const { data: asset } = await supabase
    .from("assets")
    .select("course_id, module_id, storage_path, video_poster_storage_path")
    .eq("id", parsed.data)
    .maybeSingle();
  const parentCourseId =
    asset?.course_id ??
  ...
  if (asset?.storage_path)
    await supabase.storage.from("course-media").remove([asset.storage_path]);
  if (asset?.video_poster_storage_path)
    await supabase.storage
      .from("course-media")
      .remove([asset.video_poster_storage_path]);
  const { error } = await supabase
    .from("assets")
    .delete()
    .eq("id", parsed.data);
...
export async function assignCourse(input: unknown) {
  ...
  const { data: participants } = await supabase
    ...
    .in("role", ["empleado", "franquiciado"])
    .eq("is_active", true);
  if ((participants?.length ?? 0) !== assignment.userIds.length)
    return {
      error: "Solo podés asignar cursos a empleados o franquiciados activos.",
    };
...
export async function unassignCourse(input: unknown) {
  ...
  if (!parsed.success)
    return { error: "La asignación seleccionada no es válida." };
  const supabase = await createClient();
  const { error } = await supabase
    .from("enrollments")
    .delete()
...
export async function saveExam(input: unknown) {
  ...
  if (error || !savedExam)
    return { error: error?.message ?? "No pudimos guardar el examen." };
  ...
  if (exam.id) {
    ...
    if (deleteError)
      return {
        error: "El examen se guardó, pero no pudimos actualizar sus preguntas.",
      };
  }
  for (const [index, question] of exam.questions.entries()) {
    ...
    if (questionError || !savedQuestion)
      return {
        error: "El examen se guardó, pero una pregunta no pudo registrarse.",
      };
    ...
    if (optionError)
      return {
        error: "El examen se guardó, pero una opción no pudo registrarse.",
      };
  }

...
async function resolveCourseCover(course: StoredCourseRow): Promise<CourseRow> {
  ...
  const { data: signedCover, error } = await createAdminClient()
    .storage.from("course-media")
    .createSignedUrl(coverStoragePath, 60 * 60);
  return error || !signedCover?.signedUrl
    ? publicCourse
    : { ...publicCourse, cover_url: signedCover.signedUrl };
}
...
export async function getEmployeeCourses(
  ...
  const { data: enrollmentData } = await supabase
    .from("enrollments")
    .select(
      "id, course_id, due_date, status, progress_percent, assigned_at, started_at, completed_at",
    )
  ...
  const [{ data: courseData }, { data: moduleData }] = await Promise.all([
    supabase
      .from("courses")
      .select(
        "id, title, slug, description, summary, cover_url, cover_storage_path, category, estimated_minutes, is_published",
      )
      .in("id", courseIds),
    supabase
      .from("modules")
  ...
  const courses = await Promise.all(
    ((courseData ?? []) as StoredCourseRow[]).map(resolveCourseCover),
  );
  const modules = (moduleData ?? []) as Array<
    Pick<ModuleRow, "id" | "course_id" | "order_index">
  >;
  ...
  for (const courseModule of modules) {
    moduleCountByCourse.set(
      ...
      (moduleCountByCourse.get(courseModule.course_id) ?? 0) + 1,
    );
    if (!firstModuleIdByCourse.has(courseModule.course_id))
      firstModuleIdByCourse.set(courseModule.course_id, courseModule.id);
  }
      ...
      ? [
          {
            ...
            firstModuleId: firstModuleIdByCourse.get(course.id) ?? null,
          },
        ]
      : [];
  });
...
export async function getEmployeeCourseModule(
  viewer: Viewer,
  slug: string,
  moduleId: string,
): Promise<EmployeeCourseModule | null> {
  const supabase = await createClient();
  ...
  const [
    ...
    supabase
      .from("exams")
      .select(
        "id, course_id, module_id, title, description, passing_score, max_attempts, cooldown_minutes, time_limit_minutes, shuffle_questions, shuffle_options, show_correct_answers, blocks_progress",
      )
      .eq("module_id", currentModule.id)
      .eq("is_active", true),
  ...
  const videoAssetIds = allAssets
    .filter((asset) => asset.type === "video")
    .map((asset) => asset.id);
  const resourceAssetIds = allAssets
    .filter((asset) => asset.type !== "video")
    .map((asset) => asset.id);
  const [
    { data: progressData },
    { data: resourceProgressData },
    { data: moduleProgressData },
  ] = await Promise.all([
    videoAssetIds.length
      ? supabase
          .from("video_progress")
          .select("asset_id, last_position, watched_ranges, completed")
          .eq("user_id", viewer.id)
  ...
  const progressByAssetId = new Map(
    ((progressData ?? []) as VideoProgressRow[]).map((progress) => [
      progress.asset_id,
      progress,
    ]),
  );
  const completedResourceAssetIds = new Set(
    ((resourceProgressData ?? []) as ResourceProgressRow[]).map(
      (progress) => progress.asset_id,
    ),
  );
  ...
  const accessibleAssets = await Promise.all(
    allAssets.map(async (asset) => {
      ...
      let nextAsset: AssetRow = asset;
      if (asset.storage_path) {
        const { data: signedAsset, error } = await admin.storage
          .from("course-media")
          .createSignedUrl(asset.storage_path, 60 * 60);
        if (error || !signedAsset?.signedUrl) return null;
        nextAsset = { ...nextAsset, url: signedAsset.signedUrl };
      }
      if (asset.video_poster_storage_path) {
        const { data: signedPoster } = await admin.storage
          .from("course-media")
          .createSignedUrl(asset.video_poster_storage_path, 60 * 60);
        nextAsset = {
          ...nextAsset,
          video_poster_url: signedPoster?.signedUrl ?? null,
        };
      }
      return nextAsset;
    }),
  ...
  const accessibleById = new Map(
    accessibleAssets.flatMap((asset) =>
      ...
      ? [
          {
            ...
            progress,
            isCompleted:
              accessibleAsset.type === "video"
                ? Boolean(progress?.completed)
                : completedResourceAssetIds.has(asset.id),
          },
        ]
      : [];
  };
...
export async function getEmployeeExam(
  viewer: Viewer,
  examId: string,
  ...
  const { data: questionData } = await admin
    .from("questions")
    .select("id, prompt, explanation, points, order_index")
    .eq("exam_id", exam.id)
    .order("order_index");
  ...
  return {
    ...
    questions: orderedQuestions.map((question) => ({
      ...
      options: exam.shuffle_options
        ? shuffle(optionsByQuestionId.get(question.id) ?? [])
...
export async function getEmployeeProgress(
  ...
  const [{ data: examData }, { data: moduleData }] = await Promise.all([
    attempts.length
      ? supabase
          ...
          .in(
            "id",
            attempts.map((attempt) => attempt.exam_id),
          )
      : Promise.resolve({ data: [] }),
  ...
  const courseByModuleId = new Map(
    ((moduleData ?? []) as Array<{ id: string; course_id: string }>).map(
      (courseModule) => [courseModule.id, courseModule.course_id],
    ),
  );
  ...
  return courses.map((course) => {
    const courseAttempts = attempts.filter((attempt) => {
      ...
      const relatedCourseId =
        exam?.course_id ??
        (exam?.module_id ? courseByModuleId.get(exam.module_id) : null);
      return relatedCourseId === course.id;
    });
    const scores = courseAttempts
      .map((attempt) => attempt.score)
      .filter((score): score is number => typeof score === "number");
    const completedDates = courseAttempts
    ...
    return {
      ...
      averageScore: scores.length
        ...
        : null,
      latestAttemptAt: completedDates[0] ?? null,
    };
  });
}

  ...
  return (
    <section className="overflow-hidden bg-ink">
      ...
      <div className="flex items-center justify-between gap-4 border-t border-white/15 px-4 py-3 text-sm text-white/80">
        ...
        <span className="font-tabular shrink-0">{asset.progress?.completed ? "Completado" : formatMinutes(asset.duration_seconds)}</span>
      </div>
    </section>
  );
}
...
function AssetStage({ asset }: { asset: EmployeeAsset }) {
    ...
    <div className="min-h-screen bg-surface xl:grid xl:grid-cols-[260px_minmax(0,1fr)_320px]">
      ...
      <main className="min-w-0 bg-paper">
        <div className="mx-auto w-full max-w-5xl px-5 py-8 sm:px-8 lg:px-12">
          ...
          {selectedAsset ? <div className="mt-8"><AssetStage asset={selectedAsset} key={selectedAsset.id} /></div> : <section className="mt-8 border border-dashed border-line bg-surface p-6"><BookOpen className="size-5 text-jade" aria-hidden="true" /><p className="mt-4 text-sm text-muted">Este módulo todavía no tiene contenido publicado.</p></section>}

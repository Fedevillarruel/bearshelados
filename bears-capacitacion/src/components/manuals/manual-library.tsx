...
function ManualForm({ manual, categories, onClose }: { manual?: ManualRecord; categories: ManualCategory[]; onClose: () => void }) {
  ...
  async function upload(selectedFile: File) {
    ...
    const { error: uploadError } = await createClient().storage.from("manuals").uploadToSignedUrl(uploadUrl.data.path, uploadUrl.data.token, selectedFile);
    return uploadError ? { error: "No pudimos subir el archivo." } : { data: uploadUrl.data.path };
  }

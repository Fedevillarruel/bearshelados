...
function Dialog({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-30 grid place-items-center bg-ink/40 p-5" role="presentation">
      <section className="max-h-[90vh] w-full max-w-3xl overflow-y-auto border border-line bg-paper p-5 shadow-xl sm:p-6" role="dialog" aria-modal="true" aria-labelledby="course-content-dialog-title">
        <div className="mb-6 flex items-start justify-between gap-4">
          <h2 className="text-xl font-medium" id="course-content-dialog-title">{title}</h2>
          <button className="grid size-10 place-items-center rounded-sm transition-colors hover:bg-surface" type="button" onClick={onClose} aria-label="Cerrar">
...
function AssetEditor({
  ...
  const [location, setLocation] = useState(asset?.moduleId ?? (asset?.courseId ? "course" : target));
  ...
  const targetAssets = location === "course" ? courseAssets : modules.find((module) => module.id === location)?.assets ?? [];
  ...
  function loadVideoDuration(file: File) {
    ...
    preview.onloadedmetadata = () => {
      if (Number.isFinite(preview.duration) && preview.duration > 0) setDurationSeconds(String(Math.ceil(preview.duration)));
  ...
  function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    setStoragePath(null);
    setTitle((current) => current.trim() || selectedFile.name.replace(/\.[^.]+$/, ""));
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      const response = await saveAsset({
        ...
        orderIndex: asset?.orderIndex ?? Math.max(0, ...targetAssets.map((item) => item.orderIndex)) + 1,
      });
      setSaving(false);
  ...
  return (
    <form className="grid gap-5" onSubmit={submit}>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="grid gap-2 text-sm font-medium" htmlFor="content-type">
          Tipo de contenido
          <select id="content-type" className="h-11 rounded-sm border bg-paper px-3 text-sm" value={type} onChange={(event) => changeType(event.target.value as AssetType)}>
      ...
      <label className="grid gap-2 text-sm font-medium" htmlFor="content-title">
        Título
        <input id="content-title" className="h-11 rounded-sm border bg-paper px-3 text-sm outline-none focus:border-jade" value={title} onChange={(event) => setTitle(event.target.value)} required />
      </label>

      <label className="grid gap-2 text-sm font-medium" htmlFor="content-description">
        Descripción
        <input id="content-description" className="h-11 rounded-sm border bg-paper px-3 text-sm outline-none focus:border-jade" value={description} onChange={(event) => setDescription(event.target.value)} />
      </label>

      {type === "text" ? (
        <label className="grid gap-2 text-sm font-medium" htmlFor="content-text">
          Contenido
          <textarea id="content-text" className="min-h-40 resize-y rounded-sm border bg-paper px-3 py-2 text-sm outline-none focus:border-jade" value={source} onChange={(event) => setSource(event.target.value)} required />
        </label>
      ) : (
      ...
      <div className="flex justify-end gap-3 border-t border-line pt-5">
        <button className="h-11 rounded-sm border px-4 text-sm" type="button" onClick={onClose} disabled={saving}>Cancelar</button>
        <button className="h-11 rounded-sm bg-jade px-4 text-sm font-medium text-white transition-colors hover:bg-jade-deep disabled:opacity-60" type="submit" disabled={saving}>{saving ? "Guardando" : asset ? "Guardar contenido" : "Agregar contenido"}</button>
      </div>
    </form>
  );
}

function AssetRow({ asset, onEdit, onRemove }: { asset: CourseContentAsset; onEdit: () => void; onRemove: () => void }) {
  ...
  return (
    <article className="flex flex-wrap items-center justify-between gap-4 px-4 py-3">
      ...
      <div className="flex gap-1">
        <button className="grid size-9 place-items-center rounded-sm border transition-colors hover:bg-surface" type="button" onClick={onEdit} aria-label={`Editar ${asset.title}`} title="Editar contenido"><Pencil className="size-4" aria-hidden="true" /></button>
        <button className="grid size-9 place-items-center rounded-sm border text-alert transition-colors hover:bg-[#FCEAE6]" type="button" onClick={onRemove} aria-label={`Eliminar ${asset.title}`} title="Eliminar contenido"><Trash2 className="size-4" aria-hidden="true" /></button>
      </div>
    </article>
  );
}
...
export function CourseContentStudio({
  ...
  const router = useRouter();
  const [editor, setEditor] = useState<{ asset?: CourseContentAsset; target: string } | null>(null);
  ...
  function section(assetList: CourseContentAsset[], target: string, title: string, description: string) {
    return (
      <section className="border border-line bg-paper" key={target}>
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-line px-5 py-4">
          ...
          <button className="inline-flex h-10 items-center gap-2 rounded-sm border px-3 text-sm font-medium transition-colors hover:bg-surface" type="button" onClick={() => setEditor({ target })}>
            <Plus className="size-4" aria-hidden="true" />
            Agregar contenido
          </button>
        </header>
        {assetList.length ? <div className="divide-y divide-line">{assetList.map((asset) => <AssetRow asset={asset} key={asset.id} onEdit={() => setEditor({ asset, target })} onRemove={() => remove(asset)} />)}</div> : <p className="px-5 py-6 text-sm text-muted">Todavía no hay contenidos cargados.</p>}
      </section>
    );
  }
  ...
  return (
    <div>
      <header className="border-b border-line pb-6">
        <Link href={`/admin/cursos/${course.id}`} className="inline-flex items-center gap-2 text-sm text-muted transition-colors hover:text-ink"><ChevronLeft className="size-4" aria-hidden="true" />Volver al curso</Link>
        <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-sm text-muted">Biblioteca de contenido</p>
            <h1 className="mt-2 text-[31px] font-medium leading-tight">{course.title}</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">Cargá videos, documentos y recursos de apoyo. Los archivos quedan privados y se entregan con acceso firmado.</p>
          </div>
          <button className="inline-flex h-10 items-center gap-2 rounded-sm bg-jade px-4 text-sm font-medium text-white transition-colors hover:bg-jade-deep" type="button" onClick={() => setEditor({ target: "course" })}><Plus className="size-4" aria-hidden="true" />Agregar recurso</button>
        </div>
      </header>

      {notice ? <p className="mt-5 rounded-sm bg-[#FCEAE6] px-3 py-2 text-sm text-alert" role="alert">{notice}</p> : null}

      <div className="mt-6 space-y-5">
        {section(courseAssets, "course", "Recursos generales del curso", "Disponibles para todas las personas asignadas al curso.")}
        {modules.map((module, index) => section(module.assets, module.id, `Módulo ${String(index + 1).padStart(2, "0")}: ${module.title}`, "Se muestran dentro de esta lección para las personas asignadas."))}
        {!modules.length ? <section className="border border-dashed border-line bg-surface p-6"><Video className="size-5 text-jade" aria-hidden="true" /><p className="mt-4 text-sm text-muted">Podés cargar recursos generales ahora o crear módulos para organizar lecciones y videos.</p></section> : null}
      </div>

      {editor ? <Dialog title={editor.asset ? "Editar contenido" : "Agregar contenido"} onClose={() => setEditor(null)}><AssetEditor course={course} modules={modules} courseAssets={courseAssets} asset={editor.asset} target={editor.target} onClose={() => setEditor(null)} /></Dialog> : null}
    </div>
  );
}
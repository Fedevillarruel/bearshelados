...
export function PortalLayout({ viewer, activeKey, children }: PortalLayoutProps) {
  const navigation = viewer.role === "admin" && viewer.isSuperAdmin
    ? [...navigationByRole.admin, { href: "/admin/tiendanube", label: "Tiendanube", icon: ShoppingBag, key: "tiendanube" }]
    : navigationByRole[viewer.role];
